...
public void OnPluginStart()
{
	...
	CreateConVar("sm_advancedpm_version", PLUGIN_VERSION, "Advanced Private Messages Version", FCVAR_DONTRECORD);
	...
	RegAdminCmd("sm_pmspy", Cmd_PmSpy, ADMFLAG_CHAT, "Enable/Disable Private Message Spy.");
...
public Action Cmd_PM(int client, int args)
{
	...
	for (int i = 2; i <= args; i++)
	{
		...
		Format(message, sizeof(message), "%s %s", message, arg1);
	}
...
public void OnClientDisconnect(int client)
{
	...
	replyTo[client] = -1;
	if (temp <= MAXPLAYERS && temp >= 0) replyTo[temp] = -1;
}
...
{
	if (StrStarts(sArgs, "!privatemessage ") || StrStarts(sArgs, "!pm ") || StrStarts(sArgs, "!tell ") || StrStarts(sArgs, "!msg ") || StrStarts(sArgs, "!message ") || StrStarts(sArgs, "!qr ") || StrStarts(sArgs, "!reply ")) return Plugin_Handled;

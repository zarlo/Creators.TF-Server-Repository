...
public Plugin myinfo = 
{
    ...
    author = "McFlurry, Keith Warren (Shaders Allen), and Nanochip",
...
public void OnPluginStart()
{
    ...
    ConVars[14] = CreateConVar("sm_chat_log_votekick", "1", "Log votekicks in chat log?", FCVAR_NOTIFY, true, 0.0, true, 1.0);
    ...
    AddCommandListener(VoteKick, "callvote");
...
public Action VoteKick(int client, const char[] cmd, int argc)
{
    if(!sv_bEnabled || !sv_bVoteKick || !IsHooked) return Plugin_Continue;
    ...
    GetClientName(target, Target, sizeof(Target));
    ...
    switch (sv_bFormatting)
    {
        case true: Format(sFormat, sizeof(sFormat), "[%s] \\cf1 -- %s(%s) called a votekick on %s(%s) --\\cf0", sTime, sName, sClientAuth, Target, sTargetAuth);
        case false: Format(sFormat, sizeof(sFormat), "[%s] -- %s(%s) called a votekick on %s(%s) --", sTime, sName, sClientAuth, Target, sTargetAuth);
    }
    
    char sPrint[512];
    switch (sv_bFormatting)
    {
        case true: Format(sPrint, sizeof(sPrint), "%s\\par", sFormat);
        case false: Format(sPrint, sizeof(sPrint), "%s", sFormat);
    }

...
bool g_bShowAdvert[MAXPLAYERS+1];
Handle g_Cookie;
...
public void OnPluginStart()
{
    ...
    g_Cookie = RegClientCookie("cookie_advertisements", "Show/Hide Advertisements", CookieAccess_Public);
    SetCookiePrefabMenu(g_Cookie, CookieMenu_OnOff_Int, "Chat Advertisements/Tips");
	
    for (int i = 1; i <= MaxClients; i++)
    {
        g_bShowAdvert[i] = true;
...
{
    g_bShowAdvert[client] = true;
}

public void OnClientCookiesCached(int client)
{
    char val[8];
    GetClientCookie(client, g_Cookie, val, sizeof val);
    if (StrEqual(val, "0")) g_bShowAdvert[client] = false;
}
...
public Action Timer_DisplayAd(Handle timer)
{
    ...
    if (sCenter[0]) {
        ...
        for (int i = 1; i <= MaxClients; i++) {
            if (IsClientInGame(i) && !IsFakeClient(i) && g_bShowAdvert[i] &&

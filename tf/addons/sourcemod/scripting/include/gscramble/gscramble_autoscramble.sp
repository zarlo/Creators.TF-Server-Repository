...
Description:
    Auto-scramble logic for the gscramble addon
*************************************************************************
*************************************************************************
...
Float:GetClientScrambleScore(client, e_ScrambleModes:mode)
{
    ...
    switch (mode)
    {
        ...
        case gameMe_SkillChange:
        {
            ...
            {
                return (float(g_aPlayers[client][iGameMe_SkillChange]) / GetClientTime(client));
            }
        }
        case hlxCe_Rank:
        {
        ...
        default:
        {
            ...
            if (!IsFakeClient(client))
            {
                ...
                new Float:fTime = (fClientTime / 60.0);
                fScore = (fScore / fTime);
            }
            else
            {
...
stock ScramblePlayers(e_ScrambleModes:scrambleMode)
{
    ...
    */
    ...
    {
        float scoreArray[MAXPLAYERS+1][2];
        for (i = 0; i < iCount; i++)
        {
...
PrintScrambleStats(swaps)
{
    ...
    {
        new Float:fScrPercent = float(swaps) / float(GetClientCount(true));
...
stock bool DisableScrambleImmunityCheck()
{
    ...
    if (!iTotal)
        return false;
    if ((float(iImmuneTotal) / float(iTotal)) >= fPercent)
        return true;
    return false;
}***/
